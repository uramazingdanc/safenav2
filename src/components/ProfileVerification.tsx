...
const ProfileVerification = ({ verificationStatus, adminNotes }: ProfileVerificationProps) => {
  ...
  const handleSubmit = async () => {
    if (!idFile || !selfieFile) {
      toast({ title: 'Missing Files', description: 'Please upload both your ID and selfie', variant: 'destructive' });
      return;
    }

    try {
      await submitVerification.mutateAsync({ idFile, selfieFile });
      toast({ title: 'Success', description: 'Verification submitted successfully! Please wait for admin review.' });
      handleClose();
    } catch (error: any) {
        ...
        <CardContent className="p-4">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-amber-600">Your ID and selfie are being reviewed.</p>
            </div>
          </div>
        </CardContent>
      </Card>
  ...
  if (verificationStatus === 'rejected') {
    return (
      <>
          ...
          <CardContent className="p-4">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                {adminNotes && <p className="text-sm text-red-600 mb-2"><strong>Reason:</strong> {adminNotes}</p>}
                <Button size="sm" onClick={() => setIsUploadOpen(true)} className="bg-red-600 hover:bg-red-700">
                  <Upload className="w-4 h-4 mr-2" />Try Again
                </Button>
              </div>
            </div>
          </CardContent>
        ...
        <VerificationDialog 
          isOpen={isUploadOpen} onClose={handleClose}
          idPreview={idPreview} selfiePreview={selfiePreview}
          idInputRef={idInputRef} selfieInputRef={selfieInputRef}
          onIdSelect={handleIdSelect} onSelfieSelect={handleSelfieSelect}
          onSubmit={handleSubmit} isSubmitting={submitVerification.isPending}
        ...
        <CardContent className="p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-muted-foreground mb-3">Verify your identity to unlock full SafeNav features.</p>
...
const VerificationDialog = ({
  isOpen, onClose, idPreview, selfiePreview,
  idInputRef, selfieInputRef, onIdSelect, onSelfieSelect,
        ...
        <ScrollArea className="max-h-[calc(90vh-100px)] px-6 pb-6">
          <div className="space-y-4 pt-4">
            ...
            <div className="space-y-2">
              ...
              {selfiePreview ? (
                <div 
                  className="border-2 border-primary bg-primary/5 border-dashed rounded-xl p-4 text-center cursor-pointer"
                  onClick={() => selfieInputRef.current?.click()}
                >
                  <img src={selfiePreview} alt="Selfie Preview" className="max-h-32 mx-auto rounded-lg object-contain" />
                </div>
              ) : (
                <div className="flex gap-2">
                  <div 
                    className="flex-1 border-2 border-dashed rounded-xl p-4 text-center cursor-pointer border-muted-foreground/25 hover:border-primary/50"
                    onClick={() => selfieInputRef.current?.click()}
                  >
            ...
            <div className="flex gap-2">
              ...
              <Button className="flex-1" disabled={!canSubmit || isSubmitting} onClick={onSubmit}>
                {isSubmitting ? (
                  ...
                  <><Camera className="w-4 h-4 mr-2" />Submit for Review</>
                )}
              </Button>
            </div>
          </div>
