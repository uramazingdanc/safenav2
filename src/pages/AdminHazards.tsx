...
const AdminHazards = () => {
  ...
  const filteredHazards = hazards?.filter(hazard => hazard.type.toLowerCase().includes(searchTerm.toLowerCase()) || hazard.location.toLowerCase().includes(searchTerm.toLowerCase())) || [];
  ...
  return <div className="p-4 md:p-6 space-y-6 pb-20 md:pb-6 animate-fade-in">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-primary-foreground">Hazard Management</h1>
      ...
      <div className="flex gap-2">
        <div className="relative flex-1">
          ...
          <Input placeholder="Search hazards..." className="pl-9" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        </div>
      ...
      <div className="space-y-3">
        {filteredHazards.length === 0 ? <Card>
            <CardContent className="p-8 text-center text-muted-foreground">
              {searchTerm ? 'No hazards found matching your search.' : 'No hazards reported yet.'}
            </CardContent>
          </Card> : filteredHazards.map(hazard => <Card key={hazard.id} className="hover:shadow-md transition-shadow">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded-full ${hazard.status === 'active' ? 'bg-destructive/10 text-destructive' : 'bg-muted text-muted-foreground'}`}>
                        {hazard.status}
                      </span>
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button variant="outline" size="sm" className="text-destructive" onClick={() => deleteHazard.mutate(hazard.id)} disabled={deleteHazard.isPending}>
                      Delete
                    </Button>
                  </div>
                </div>
